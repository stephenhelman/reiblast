...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (locationId) {
      ...
      console.log('[Provision] Searching for sub-account for:', user.businessName)
      locationId =
      ...
      if (!locationId) {
        console.error(`[Provision] Could not find sub-account for ${user.businessName}`)
    ...
    console.log('[Provision] Populating sub-account custom values:', locationId)
    ...
    console.error(`[Provision] Error contactId=${contactId} email=${normalizedEmail}:`, err)
    return NextResponse.json(
      {
