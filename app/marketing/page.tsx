    ...
    <>
      ...
      <section className="py-16 px-6 border-y border-border-default bg-surface">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-6">
          {PAIN_POINTS.map((text) => (
            <div key={text} className="bg-surface border-l-4 border-gold rounded-r-xl px-6 py-5">
      ...
      <section className="py-20 px-6 bg-surface border-b border-border-default">
        <div className="max-w-4xl mx-auto">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            {STATS.map((s) => (
              <div key={s.number}>
                <div className="text-6xl font-extrabold text-gold mb-3">{s.number}</div>
      ...
      <section className="py-24 px-6 bg-surface border-b border-border-default text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mt-4">
            <Link href="/pricing" className="text-white/40 text-sm hover:text-white/70 transition-colors">
              See full pricing →
            </Link>
          </div>
        </div>
