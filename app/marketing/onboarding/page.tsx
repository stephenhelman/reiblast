...
type GateState = 'checking' | 'waiting' | 'not_found' | 'active' | 'onboarding_complete' | 'pending_onboarding'
...
function OnboardingContent() {
    ...
    async function poll() {
      try {
        ...
        if (res.status === 404) {
          ...
          setReconcileMsg(RECONCILE_MESSAGES[attempt % RECONCILE_MESSAGES.length])
  ...
  if (gateState === 'waiting') {
    const pct = Math.min(100, Math.round((reconcileAttempt / RECONCILE_MAX_ATTEMPTS) * 100))
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center py-16 px-4">
        <div className="max-w-md w-full">
          ...
          <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-8 text-center">
            ...
            <h2 className="text-white font-semibold text-lg mb-3">{reconcileMsg}</h2>
