  ...
  try {
    ...
    if (hostname === "realtor.com") {
      const match = url.pathname.match(
        /\/realestateandhomes-detail\/(.+?)(_M[\d]+)?$/,
      );
      if (match) {
...
function LandContent() {
  ...
  const filteredComps = useMemo<LandComp[]>(() => {
    return allComps.filter((c) => {
      ...
      if (
        filters.days !== null &&
        c.daysSinceSold !== null &&
        c.daysSinceSold > filters.days
      )
        return false;
      if (
        filters.lotSizeRange !== null &&
        subjectAcres > 0 &&
        c.lotSizeAcres !== null
      ) {
      ...
      if (filters.lotType !== "all" && c.lotSizeAcres !== null) {
        ...
        if (
          filters.lotType === "acreage" &&
          (c.lotSizeAcres < 1 || c.lotSizeAcres > 10)
        )
          return false;
  ...
  const sortedTableComps = useMemo(() => {
    ...
    return [...tableComps].sort((a, b) => {
      switch (sortCol) {
        ...
        case "acres":
          return dir * ((a.lotSizeAcres ?? 0) - (b.lotSizeAcres ?? 0));
        case "sold":
          return (
            dir * ((a.daysSinceSold ?? 999999) - (b.daysSinceSold ?? 999999))
          );
        case "dist":
          return dir * (a.distanceMiles - b.distanceMiles);
        case "price":
          return dir * ((a.lastSalePrice ?? 0) - (b.lastSalePrice ?? 0));
        default:
          return 0;
      }
    });
  ...
  useEffect(() => {
    ...
    setLoadingDeal(true);
    fetch(
      `/api/analyzer/land-deal?dealId=${encodeURIComponent(dealId)}&locationId=${encodeURIComponent(locationId)}`,
    )
      .then((r) => r.json())
      .then((deal) => {
          ...
          try {
            ...
            const fresh = rawCompsData.map((c) => ({
              ...
              daysSinceSold: c.lastSaleDate
                ? Math.floor(
                    (Date.now() - new Date(c.lastSaleDate).getTime()) /
                      86400000,
                  )
                : null,
            }));
        ...
        const reconstructed: LandAnalysisResult = {
          ...
          estimated_value: {
            ...
            confidence:
              (deal.valueConfidence as "high" | "medium" | "low") ?? "medium",
            confidence_reason: "",
            value_method:
              (deal.valueMethod as "comps" | "tax_assessment" | "hybrid") ??
              "comps",
          },
          builder_activity: {
            level:
              (deal.builderActivityLevel as
                | "high"
                | "moderate"
                | "low"
                | "unknown") ?? "unknown",
          ...
          exit_strategy: {
            recommendation:
              (deal.exitStrategy as LandAnalysisResult["exit_strategy"]["recommendation"]) ??
              "RETAIL_LOT_BUYER",
            reasoning: "",
          },
  ...
  const updateCompMarkers = useCallback(() => {
    ...
    allComps.forEach((comp) => {
      ...
      const color = isManual ? "#f59e0b" : isSelected ? "#22c55e" : "#eab308";
  ...
  function handleManualCompUrlBlur() {
    ...
    if (parsed) {
      ...
      manualCompUrl.trim() &&
      manualCompUrl.trim().startsWith("http")
    ) {
      setManualCompUrlError(
        "Could not parse URL. Enter the address manually below.",
      );
    }
  }
  ...
  function validateStep2(): boolean {
    ...
    if (landInfo.lotSizeAcres === "")
      errs.lotSizeAcres = "Lot size is required";
        ...
        {step === 1 && (
          <div className="max-w-xl mx-auto">
            <h2 className="text-xl font-bold mb-2 text-center">
              Enter the land address
            </h2>
            <p className="text-white/40 text-sm mb-8 text-center">
              We&apos;ll pull comparable land sales in the area.
            </p>
        ...
        {step === 2 && place && (
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-white/40 text-sm mb-6">
              Fill in what you know. More detail = more accurate analysis.
            </p>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                value={
                  lotSizeUnit === "acres"
                    ? landInfo.lotSizeAcres === ""
                      ? ""
                      : landInfo.lotSizeAcres
                    : landInfo.lotSizeSqft === 0
                      ? ""
                      : landInfo.lotSizeSqft
                }
                onChange={(e) => {
                  ...
                  if (lotSizeUnit === "acres") {
                    ...
                    setLandInfo((prev) => ({
                      ...
                      lotSizeSqft: sqft,
                      lotSizeAcres:
                        sqft > 0 ? parseFloat((sqft / 43560).toFixed(2)) : "",
                    }));
                  }
                  setValidationErrors((prev) => ({
                    ...prev,
              ...
              {typeof landInfo.lotSizeAcres === "number" &&
                landInfo.lotSizeAcres > 0 && (
            ...
            <p className="text-white/30 text-xs mb-4 italic">
              Helps improve analysis (optional)
            </p>
            ...
            <div className="mb-5">
              ...
              <input
                ...
                onChange={(e) =>
                  setLandInfo((p) => ({ ...p, zoning: e.target.value }))
                }
          ...
          <div className="relative flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        radius: Number(e.target.value) as LandFilters["radius"],
                      }))
                    }
                    ...
                    onChange={(e) => {
                      ...
                      setFilters((f) => ({
                        ...
                        days:
                          v === ""
                            ? null
                            : (Number(v) as LandFilters["days"] & number),
                      }));
                    }}
                    className="bg-surface-2 text-white text-xs border border-border-default rounded-lg px-2 py-1.5 outline-none cursor-pointer focus:border-gold w-full"
                  >
                    ...
                    onChange={(e) => {
                      ...
                      setFilters((f) => ({
                        ...
                        lotSizeRange:
                          v === ""
                            ? null
                            : (Number(v) as LandFilters["lotSizeRange"] &
                                number),
                      }));
                    }}
                    className="bg-surface-2 text-white text-xs border border-border-default rounded-lg px-2 py-1.5 outline-none cursor-pointer focus:border-gold w-full"
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilters((f) => ({
                        ...f,
                        lotType: e.target.value as LandFilters["lotType"],
                      }))
                    }
                    className="bg-surface-2 text-white text-xs border border-border-default rounded-lg px-2 py-1.5 outline-none cursor-pointer focus:border-gold w-full"
                  >
            ...
            {!fetchingComps && selectedCount === 0 && (
              <div className="bg-yellow-400/10 border border-yellow-400/30 rounded-xl px-4 py-3 text-yellow-400 text-sm">
                No comps selected. Analysis will use tax assessment data only
            ...
            {showAddComp && (
              <div className="bg-surface border border-border-default rounded-xl p-4 space-y-3">
                ...
                <div>
                  <label className="block text-white/30 text-xs mb-1">
                    Paste Zillow, Redfin, or Realtor.com URL (optional)
                  </label>
                  <input
                    type="url"
                    value={manualCompUrl}
            ...
            {fetchingComps ? (
              ...
              <p className="text-white/30 text-sm text-center py-10">
                No comparable land sales found. Try expanding filters or add
                comps manually.
              </p>
            ) : (
                    ...
                    <tr className="border-b border-border-default bg-surface">
                      ...
                      {[
                        ...
                        <th
                          key={key}
                          onClick={() =>
                            key !== "zoning" ? handleSort(key) : undefined
                          }
                        ...
                        >
                          ...
                          <td className="p-2">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {isManual && (
                                <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-amber-400/15 text-amber-400 font-medium">
                                  Manual
                                </span>
                              )}
                              {isOutsideFilter && !isManual && (
                                <span className="shrink-0 text-[10px] px-1 py-0.5 rounded bg-yellow-400/10 text-yellow-400 font-medium">
                                  outside filter
                                </span>
                              )}
                            </div>
                          ...
                          <td className="p-2 text-right text-[13px] text-white/80">
                            {comp.lotSizeAcres != null
                              ? comp.lotSizeAcres.toFixed(2) + " ac"
                              : "—"}
                          </td>
                          ...
                          <td className="p-2 text-[13px] text-white/80">
                            {comp.daysSinceSold != null
                              ? relativeDate(comp.daysSinceSold)
                              : "—"}
                          </td>
                          <td className="p-2 text-right text-[13px] text-white/80">
            ...
            >
              <div>
                {analyzing ? (
                  ...
                  <p className="text-white/50 text-sm">
                    {selectedCount} comp{selectedCount !== 1 ? "s" : ""}{" "}
                    selected
                  </p>
                )}
              </div>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-surface border border-gold rounded-xl p-5">
                        ...
                        <p className="text-white/40 text-xs mb-1">
                          {fmt(analysis.estimated_value.low)} —{" "}
                          {fmt(analysis.estimated_value.high)}
                        </p>
                        <p className="text-white text-2xl font-bold mb-2">
                          {fmt(analysis.estimated_value.estimate)}
                        </p>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          <span
                            className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${
                              analysis.estimated_value.confidence === "high"
                                ...
                                : analysis.estimated_value.confidence ===
                                    "medium"
                          ...
                          <span className="inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-white/5 text-white/50 capitalize">
                            {analysis.estimated_value.value_method.replace(
                              "_",
                              " ",
                            )}
                          </span>
                        ...
                        <p className="text-white/30 text-[11px] italic leading-relaxed">
                          {analysis.estimated_value.confidence_reason}
                        </p>
                      </div>
                      ...
                      <div className="bg-surface border border-border-default rounded-xl p-5">
                        ...
                        <p className="text-white/60 text-xs leading-relaxed">
                          {analysis.builder_activity.note}
                        </p>
                      </div>
                    </div>
                      ...
                      {[
                        {
                          ...
                          value:
                            subjectAcres > 0
                              ? `${subjectAcres.toFixed(2)} ac`
                              : null,
                        },
                        { label: "Zoning", value: landInfo.zoning || null },
                        {
                          label: "Road Access",
                          value: landInfo.roadAccess || null,
                        },
                      ].map(({ label, value }) => (
                        <div
                          key={label}
                        ...
                        >
                          ...
                          <p
                            className={`font-semibold text-sm ${value ? "text-white" : "text-white/30"}`}
                          >
                            {value ?? "Not provided"}
                          </p>
                        </div>
                    ...
                    <div className="space-y-2 pt-2 border-t border-border-default">
                      <p className="text-white/25 text-[11px] leading-relaxed">
                        ⓘ AriAI can make mistakes. Always verify estimated
                        values and comp data with your own due diligence before
                        making an offer on any land deal.
                      </p>
                      <p className="text-white/25 text-[11px] leading-relaxed">
                        ⓘ Limited sale data is common in land markets. When
                        fewer than 3 comps with actual sale prices are
                        available, treat the estimated value as directional
                        only.
                      </p>
                    </div>
                ...
                <div className="bg-surface border border-border-default rounded-xl overflow-hidden">
                  <div className="divide-y divide-border-default">
                    ...
                    <div className="flex items-center justify-between px-5 py-3.5 pl-8">
                      ...
                      <span className="text-white font-semibold text-sm">
                        {fmt(
                          analysis.estimated_value.estimate *
                            calcResults.keepPct,
                        )}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between px-5 py-3.5">
                      <div>
                        ...
                        <p className="text-white/30 text-xs mt-0.5">
                          Start here — gives you room to negotiate up to cash
                          offer
                        </p>
                      </div>
